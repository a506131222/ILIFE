...
@interface ZJMWebView () <WKNavigationDelegate, WKUIDelegate>
/// WKWebView
...
- (void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation {
    if (self.ZJMQRCodeDelegate && [self.ZJMQRCodeDelegate respondsToSelector:@selector(webViewDidStartLoad:)]) {
        [self.ZJMQRCodeDelegate webViewDidStartLoad:self];
    }
}
/// 当内容开始返回时调用
- (void)webView:(WKWebView *)webView didCommitNavigation:(WKNavigation *)navigation {
    if (self.ZJMQRCodeDelegate && [self.ZJMQRCodeDelegate respondsToSelector:@selector(webView:didCommitWithURL:)]) {
        [self.ZJMQRCodeDelegate webView:self didCommitWithURL:self.wkWebView.URL];
    }
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    self.navigationItemTitle = self.wkWebView.title;
    if (self.ZJMQRCodeDelegate && [self.ZJMQRCodeDelegate respondsToSelector:@selector(webView:didFinishLoadWithURL:)]) {
        [self.ZJMQRCodeDelegate webView:self didFinishLoadWithURL:self.wkWebView.URL];
    }
...
- (void)webView:(WKWebView *)webView didFailNavigation:(WKNavigation *)navigation withError:(NSError *)error {
    if (self.ZJMQRCodeDelegate && [self.ZJMQRCodeDelegate respondsToSelector:@selector(webView:didFailLoadWithError:)]) {
        [self.ZJMQRCodeDelegate webView:self didFailLoadWithError:error];
    }
